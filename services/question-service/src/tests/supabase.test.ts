...
describe('supabaseClient', () => {
  ...
  it('falls back to environment variables when secret files are missing', async () => {
    ...
    expect(mockCreateClient).toHaveBeenCalledWith('https://env-url.supabase.co', 'env-service-key');
  });
  ...
  it('throws when neither secret files nor environment variables are set', async () => {
    ...
    await expect(import('../lib/supabase')).rejects.toThrow('Missing Supabase credentials');
  });
