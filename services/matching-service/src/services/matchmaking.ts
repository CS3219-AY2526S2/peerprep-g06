...
export async function findMatch(user: User): Promise<Match | undefined> {
  ...
  for (const candidate of candidates) {
    ...
    if (!isLocked) {
      logger.debug(
        `Could not lock ${user.id} or ${candidate.id}, skipping candidate ${candidate.id}`,
      );
      continue;
    }
    ...
    try {
      ...
      if (
        !userRequest ||
        userRequest.status !== MatchStatus.PENDING ||
        !candidateRequest ||
        candidateRequest.status !== MatchStatus.PENDING
      ) {
        logger.debug(
          `User state changed before locks were acquired, skipping candidate ${candidate.id}`,
        );
        continue;
      }
